...
def load_data_img(args):
    problems = json.load(open(os.path.join(args.data_root, 'scienceqa/problems.json')))
    pid_splits = json.load(open(os.path.join(args.data_root, 'scienceqa/pid_splits.json')))
    #problems = json.load(open('/data/mm-cot-main/random_extracted_data_240000.json'))
    #pid_splits = json.load(open('/data/mm-cot-main/random_pid_splits.json'))
    captions = json.load(open(args.caption_file))["captions"]
    name_maps = json.load(open('vision_features/name_map.json'))
    #name_maps = json.load(open('prefeature_random/name_map.json'))
    ...
    if args.img_type == "resnet":
        ...
        #image_features = np.load('prefeature_random/detr.npy')
...
class ScienceQADatasetStd(Dataset):
    ...
    def __getitem__(self, index):
        ...
        target_ids = torch.tensor(target["input_ids"].squeeze().tolist()).to(device)
    ...
    def __init__(
        ...
        for qid in self.data:
            ...
            if str(qid) in name_maps:
                ...
                self.image_ids.append(torch.tensor(i_vectors).to(device))
            else:
                shape = img_shape[args.img_type]
                self.image_ids.append(torch.zeros(shape).to(device))
